...
Story Maker stitches several text blocks, each with its own voice, speed, and emotion, into a single narrated audio file. Handy for multi-character stories or any script that needs different voices in sequence.

## Text To Speech
`POST https://api.ttsopenai.com/uapi/v1/story-maker`

Converts a list of text blocks into one combined speech file. Each block can have its own voice, speed, and model.
